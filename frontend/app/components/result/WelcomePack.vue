...
// Result → Welcome tab body: a white lane (the ONE scroll area) with the
// two-level grouping — category (the batch header line) → element → a 6-per-row
// image grid — and the shared right panel (Edit / Scale / Download), like every
// other Result tab. All logic lives in useWelcomePack (unit-tested); this file
// is the template. Mirrors result/TournamentPack.vue minus the Base/VIP mode.
import {
  useWelcomePack,
  groupPackByElement,
  packDisplayName,
...
const {
  ...
  api: useApi() as unknown as WelPackApi,
...
// else the single ticked image (panelScaleImage).
...
// finished image in display order (batch → element → grid).
...
<template>
  <div class="pack">
    <div class="content">
      <!-- lane: the ONE scroll area -->
      <div class="lane">
        <!-- states -->
        <p v-if="!batches.length && loading" class="lane__state">Загрузка…</p>
        <p v-else-if="!batches.length" class="lane__state">
          Пока нет генераций Welcome — запустите их на странице Welcome packs.
        </p>

        <!-- batches, newest first; header row = the category line -->
        <section v-for="b in batches" :key="b.id" class="batch">
          ...
          <!-- elements → the grid. Failed rows are hidden (retried once by the
               worker; a second failure drops the card and the counters read
               e.g. "7 of 7"). -->
            ...
            <div class="grid">
              ...
              >
                <template v-if="img.status === 'DONE' && img.generatedImageUrl">
                  ...
                  />
                  <!-- Archive-style pill with the ZIP file name -->
      ...
      <!-- right panel: Edit prompt(s) + Edit + Scale + Download -->
      <aside class="panel">
        <div class="example">
        ...
        <!-- DES ZIP export of the ticked images -->
        <button
          class="dl-btn"
          type="button"
...
/* lane + 198px right panel, 32px gutter — same frame as the Tournament tab */
.content {
  flex: 1;
  min-height: 0;
  display: grid;
...
/* the ONE scroll area of the tab (white container, thin scrollbar) */
.lane {
...
/* element block: title line, grid 14px under it */
.sub {
  display: flex;
  flex-direction: column;
  gap: 14px;
...
/* image grid — 6 per row, 8px gaps, square cards */
.grid {
  display: grid;
...
/* ---- right panel (mirrors the other tabs' edit panel) ---- */
.panel {
  display: flex;
  flex-direction: column;
  gap: 18px;
...
/* DES ZIP export — pinned to the panel bottom */
.dl-btn {
  margin-top: auto;
  display: inline-flex;
  align-items: center;
