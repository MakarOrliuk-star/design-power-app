...
/**
 * «Edit Welcome packs» (TASK welcome-packs): the super-designer's Welcome
 * editor, opened from the user menu next to «Edit Tournament pack» and built on
 ...
 * which writes the name, the prompt, own references and the active flag in ONE
 * audited call. Position (↑/↓), deletion and the own-references checkbox apply
 * immediately — they are not text, and drafting them would desync the list.
 *
 * Two differences from the tournament window: one prompt per element (no
 * Base/VIP), and reference slots appear because the CATEGORY says so, not
 * because of a hardcoded key — Welcome categories are created by hand here.
 */

const store = useWelcomePackEditorStore();
...
const catName = ref("");
/** Draft of the global system wrapper — saved by its own button. */
...
const showRefs = computed(() => (category.value ? hasOwnRefs(category.value) : false));
...
watch(
  ...
  async (open) => {
    ...
    try {
      ...
      packMsg.value = "Не удалось загрузить Welcome packs.";
    } finally {
      loading.value = false;
    }
  },
  ...
  if (
    !confirm(
      `Удалить элемент «${element.value.name}»?\n` +
        "Он пропадёт со страницы Welcome packs у всех дизайнеров. " +
...
async function addElement() {
  ...
  packMsg.value = "";
  try {
    const id = await store.createElement(category.value.id, newName.value.trim());
    newName.value = "";
    await reload();
    selectedElementId.value = id;
    syncDraft();
    packMsg.value = "Элемент добавлен ✓ Заполните промпт справа.";
  } catch (e) {
...
/**
 * The own-references checkbox — applied immediately. Turning it OFF keeps the
 * uploaded images in the DB: they stop reaching generation and come back if it
 * is turned on again, so the click is reversible.
 */
async function toggleOwnRefs(e: Event) {
  if (!category.value) return;
  const checked = (e.target as HTMLInputElement).checked;
  packMsg.value = "";
  try {
    await store.setCategoryOwnRefs(category.value.id, checked);
    await reload();
    packMsg.value = checked
      ? "Категория использует свои референсы ✓ Загрузите их у элементов."
      : "Категория снова использует референсы бренда (картинки элементов сохранены).";
  } catch (e2) {
    packMsg.value = errorOf(e2, "Не удалось изменить настройку референсов.");
    await reload(); // put the checkbox back in sync with the server
  }
}
...
async function addCategory() {
  ...
  packMsg.value = "";
  try {
    const cat = await store.createCategory(newCatName.value.trim(), newCatOwnRefs.value);
    newCatName.value = "";
    newCatOwnRefs.value = false;
    await reload();
...
async function saveSystem() {
  ...
  if (!confirm("Системная обёртка влияет на ВСЕ генерации Welcome packs. Сохранить?")) return;
  systemMsg.value = "";
  try {
...
// ---- Own references (draft until «Сохранить») ----
...
<template>
  <Teleport to="body">
    <div v-if="store.modalOpen" class="overlay" @click.self="close">
      <div class="modal" role="dialog" aria-modal="true" aria-label="Edit Welcome packs">
        ...
        <!-- ==== Left: the pack ==== -->
        <section class="col">
          <h2 class="col__title">Welcome packs</h2>
          <p class="col__sub">
            Категории, элементы и промпты страницы Welcome packs. Ничего не создаётся
            автоматически — начните с категории. Всё, что вы здесь сохраняете, применяется
            глобально для всех дизайнеров; у тех, кто правил промпт локально, появится
            плашка «дефолт изменился». Каждое изменение логируется.
          </p>
          ...
          <div class="scroll">
            <label class="field">
              ...
              >
                <option value="" disabled>
                  {{ loading ? "Загрузка…" : store.categories.length ? "Выберите категорию" : "Категорий нет — создайте ниже" }}
                </option>
            ...
            <template v-if="category">
              <div class="badges">
                ...
                <span class="badge">Элементов: {{ elements.length }}</span>
              </div>
              ...
              <!-- The заказчик's checkbox: own references instead of the brand's. -->
              <label class="check">
                <input
                  type="checkbox"
                  :checked="category.usesOwnReferences"
                  @change="toggleOwnRefs"
                />
                <span>
                  Свои референсы у элементов (вместо картинок бренда). Выключение не удаляет
            ...
            <form class="row row--add" @submit.prevent="addCategory">
              ...
              <label class="check check--inline">
                <input v-model="newCatOwnRefs" type="checkbox" />
                <span>свои рефы</span>
              </label>
              ...
              <ul class="list">
                ...
                >
                  <button class="item__pick" type="button" @click="selectElement(el.id)">
                    ...
                    <span v-if="!el.prompt" class="item__badge">без промпта</span>
            ...
            <div ref="sysEl" class="sys">
              ...
              <div v-if="systemOpen" class="sys__body">
                <p class="col__sub">
                  Текстовая добавка к промпту элемента, общая для ВСЕХ генераций Welcome packs.
                  Пусто = промпт элемента уходит prompt writer'у как есть.
                </p>
        ...
        <section :class="['col', { 'col--off': !draft }]">
          ...
          <p class="col__sub">
            {{
              draft
                ? "Название и промпт остаются черновиком, пока вы не нажмёте «Сохранить (для всех)»."
          ...
          <form v-if="draft && category" class="form" @submit.prevent="saveElement">
            ...
            <label class="field field--grow">
              <span class="field__label">Промпт</span>
              <textarea
            ...
            <div v-if="showRefs" class="field">
              <span class="field__label">Свои референсы (2)</span>
              <div class="refs">
            ...
            <div class="field">
              ...
              <label class="check">
                ...
                <span>Активен (виден на странице Welcome packs)</span>
              </label>
            </div>
...
<style scoped>
/* Same frame and tokens as TournamentPackModal.vue — one family of windows. */
.overlay {
  position: fixed;
  inset: 0;
  z-index: 100;
...
/* Flex children shrink by default, which would squeeze (and clip) the element
   list instead of letting the column scroll. */
.scroll > * {
  flex: 0 0 auto;
}
...
/* Same rule as the pack column, minus the prompt field — that one is SUPPOSED
   to absorb the leftover height. */
.form > *:not(.field--grow) {
  flex: 0 0 auto;
}
.field {
